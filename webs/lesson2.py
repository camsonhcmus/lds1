...
def app():
	...
	st.markdown("## Với các con số từ 1 đến 50, hãy tìm con số chia hết")

	st.image(Image.open('./picture/phepchia.jpg'), width=500)

	inpd = st.text_input("Hãy nhập con số chia muốn thực hiện phép chia: ")

	if st.button("Thực hiện phép chia"):
		try:
			st.write("Những con số chia hết là:")
			for i in range (1, 51):
				inpd = int(inpd)
				resultd = i / inpd
				mol = i % inpd
